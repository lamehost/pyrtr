...
def unserialize(version: int, buffer: bytes, validate: bool = True) -> IPv6Prefix:
    ...
    try:
        ...
        raise CorruptDataError("Unable to unpack the IPv6 Prefix PDU") from error

    # Handle the IP network address
    prefix_value = int.from_bytes(buffer[12:28], "big")
    ...
    if validate:
        ...
        if fields[1] != TYPE:
            raise CorruptDataError("Not a valid IPv6 Prefix PDU.")
